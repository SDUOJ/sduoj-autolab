...
class OjSign(Base):
    ...
    sg_id = Column(INTEGER, primary_key=True,autoincrement=True,description="签到id")
    # 签到模式
    mode = Column(INTEGER, nullable=False,description="签到模式")
    # 用户组id
    group_id = Column(INTEGER, nullable=False,description="用户组Id")
    # 管理组id
    m_group_id = Column(INTEGER, nullable=False,description="管理组id")
    # 创造时间
    u_gmt_create =  Column(DATETIME, nullable=False, server_default=func.now(),description="创建时间")
    # 修改时间
    u_gmt_modified = Column(DATETIME, nullable=False,description="修改时间")
    # 签到标签
    title = Column(VARCHAR(63), nullable=False,description="签到标签")
    # 开始时间
    start_time = Column(DATETIME, nullable=False,description="签到开始时间")
    # 结束时间
    end_time = Column(DATETIME, nullable=False,description="签到结束时间")
    # 是否指定座位
    # 1 绑定  0 未绑定
    seat_bind = Column(INTEGER, nullable=False,description="是否绑定座位：1 绑定  0 未绑定")
    # 名单id
    usl_id = Column(INTEGER, nullable=False,description="名单id")

# 用户签到表
class OjSignUser(Base):
    __tablename__ = "oj_sign_user"
    # 学生签到id
    sg_u_id = Column(INTEGER, primary_key=True, autoincrement=True, description="学生签到id")
    # 座位号
    seat_id = Column(INTEGER, unique=True, nullable=False, description="座位号")
    # 学生用户名
    username = Column(VARCHAR(63), unique=True, nullable=False, description="学生用户名")
    # 签到id
    sg_id = Column(INTEGER, ForeignKey("oj_sign.sg_id"), nullable=False,index=True,description="签到id")
    # 签到时间
    sg_time = Column(DATETIME, nullable=False,description="签到时间")
    # 签到唯一凭证
    token = Column(VARCHAR(63), nullable=False, unique=True, description="签到唯一凭证")
    # 请假信息
    sg_user_message = Column(LONGTEXT, nullable=True,description="请假信息")
    # 是否通过审批
    # 1 通过  0 未通过  none 审批中
    sg_absence_pass = Column(INTEGER, nullable=True,description="审批信息：1 通过  0 未通过  none 审批中")
